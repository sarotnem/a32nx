...
class FacilityLoader {
        ...
        if (this._pendingGetFacilityCoherentCall.length > 0) {
            for (let i = 0; i < 5; i++) {
                if (this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
                    const coherentCall = this._pendingGetFacilityCoherentCall.splice(0, 1)[0];
                    if (coherentCall) {
                        coherentCall();
                    }
                }
    ...
    addFacility(_data, type) {
        ...
        const facilityIndex = this.loadedFacilities.findIndex(f => f.icao === _data.icao && ((f.routes === undefined && _data.routes === undefined) || (f.routes !== undefined && _data.routes !== undefined)));
        const facilityLoadedAlready = facilityIndex > -1;
                ...
                this.instrument.requestCall(() => {
                    SimVar.SetSimVarValue("C:fs9gps:IcaoSearchEnterChar", "string", airportIdent, this.instrument.instrumentIdentifier + "-loader").then(() => {
                        SimVar.SetSimVarValue("C:fs9gps:IcaoSearchMatchedIcao", "number", 0, this.instrument.instrumentIdentifier + "-loader").then(async () => {
                            const airportIcao = SimVar.GetSimVarValue("C:fs9gps:IcaoSearchCurrentIcao", "string", this.instrument.instrumentIdentifier + "-loader");
    ...
    async GetAirportNamedFrequencies(airportIcao) {
        ...
        return new Promise(resolve => {
            SimVar.SetSimVarValue("C:fs9gps:WaypointAirportICAO", "string", airportIcao, this.instrument.instrumentIdentifier + "-loader").then(async () => {
                ...
                const getFrequency = async (index) => {
                    return new Promise((resolve) => {
                        SimVar.SetSimVarValue("C:fs9gps:WaypointAirportCurrentFrequency", "number", index, this.instrument.instrumentIdentifier + "-loader").then(() => {
                            const frequencyName = SimVar.GetSimVarValue("C:fs9gps:WaypointAirportFrequencyName", "string", this.instrument.instrumentIdentifier + "-loader");
                            const frequencyValue = SimVar.GetSimVarValue("C:fs9gps:WaypointAirportFrequencyValue", "number", this.instrument.instrumentIdentifier + "-loader");
                            resolve({
                                name: frequencyName,
                ...
                for (let i = 0; i < frequencyCount; i++) {
                    const frequency = await getFrequency(i);
